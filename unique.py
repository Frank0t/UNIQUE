...
# 
#
# Usage		 : ./unique.py example.com (or) python unique.py example.com
# Description: This scanner automates the process of security scanning by using scripts
#              
#
...
def helper():
        ...
        print "------------"
        print "\t./unique.py example.com: Scans the domain example.com"
        print "\t./unique.py --update   : Updates the scanner to the latest version."
        print "\t./unique.py --help     : Displays this help context."
        print bcolors.OKBLUE+"Interactive:"+bcolors.ENDC
        print "------------"
        print "\tCtrl+C: Skips current test."
        print "\tCtrl+Z: Quits tool."
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
    	logo()
        print "Unique is updating....Please wait.\n"
        spinner.start()
        ...
        if oldversion_hash == newversion_hash :
            clear()
            print "\t"+ bcolors.OKBLUE +"You already have the latest version of this tool." + bcolors.ENDC
        else:
            clear()
        ...
        while (rs_avail_tools < len(tools_precheck)):
			...
			try:
				...
				print "\t"+bcolors.BG_ERR_TXT+"Unique was terminated abruptly..."+bcolors.ENDC
				sys.exit(1)
			if "not found" in val:
        ...
        if len(rs_vul_list)==0:
        	...
        	with open(" Vulnerability-Report", "a") as report:
        		while(rs_vul < len(rs_vul_list)):
	       		...
	       		print "\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory RapidScan resides."
